...
def get_config(map_size, minimap_mode, tag_penalty):   #get_config는 한번만 실행됨
    gw = magent2.gridworld
    cfg = gw.Config()

    cfg.set({"map_width": map_size, "map_height": map_size})
    cfg.set({"minimap_mode": minimap_mode})
    cfg.set({"embedding_size": 10})     #Config클래스에 set을 보면, config_dict에 딕셔너리로 저장하는 것
    ...
    predator = cfg.register_agent_type("predator", options)   #cfg의 agent_type_dict에 predator와 option을 저장한다. 그리고 predator변수에는
                                                                    #"predator"을 저장한다.
    ...
    predator_group = cfg.add_group(predator)        #predator_group에는 predator 와 prey 수의 합계를 저장한다. 1이 저장
    prey_group = cfg.add_group(prey)                #prey_group에는 predator 와 prey 수의 합계를 저장한다.     2가 저장
                                                    #왜 동일한 수를 저장하는 걸까? 하긴 위에서 predator와 prey의 수를 동일하게 만들긴했다. 그냥 집단을 등록하는 것
    a = gw.AgentSymbol(predator_group, index="any") #a에는 agent(0,-1)
    b = gw.AgentSymbol(prey_group, index="any")     #b에는 agent(1,-1)   근데 이건 한번만 실행됨

    cfg.add_reward_rule(gw.Event(a, "attack", b), receiver=[a, b], value=[1, -1])
                                                    # def add_reward_rule(self, on, receiver, value, terminal=False):
                                                    #on=gw.Event(a, "attack", b)
                                                    #a:프레데터 b:먹이
                                                    #add_reward_rule(self, on, receiver, value, terminal=False)

    return cfg


class _parallel_env(magent_parallel_env, EzPickle):   #parallel_env(map_size, max_cycles, minimap_mode, extra_features, **reward_args)
    metadata = {
    ...
    def __init__(
        ...
        assert map_size >= 7, "size of map must be at least 7"     #GridWorld의 def __init__(self, config, **kwargs):
        env = magent2.GridWorld(                                   #get_config(map_size, minimap_mode, tag_penalty)
            get_config(map_size, minimap_mode, **reward_args), map_size=map_size
        )

        handles = env.get_handles()       #c언어로 작성된 두 집단을 저장한다.
        reward_vals = np.array([1, -1, -1, -1, -1] + list(reward_args.values()))  #....뭘까?
        reward_range = [
    ...
    def generate_map(self):                 #gridworld파일에 class config에도 add_agent가 있고, class Gridworld에도 add_group이 있
